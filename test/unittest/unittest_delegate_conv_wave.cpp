...
protected:
  ...
  std::string LoadDelegateInt4KernelSource() {
    const char *paths[] = {
      "delegate_conv_int4_wave.cl",
      "nntrainer/tensor/cl_operations/cl_kernels/delegate_conv_int4_wave.cl",
      "/data/local/tmp/nntr_android_test/delegate_conv_int4_wave.cl",
    };
    for (auto p : paths) {
      std::ifstream f(p);
      if (f.good())
...
// ============================================================================
// CPU-side repack: packed_weights[(K/4)*N] int4 → delegate_conv_int4_wave's
// wave-load layout. Each (Z=out/32, it=0..K/8-1) block occupies 256 ushorts;
// only the first 64 are meaningful (rows 2*it and 2*it+1 of the source,
// 32 ushorts each covering the Z*32 .. Z*32+31 output-channel window).
// Remaining 192 ushorts per block are padding so that the qcom wave load
// stride matches fp16 delegate's 32-half8 granularity.
// ============================================================================
static void RepackInt4ForDelegate(const uint16_t *packed, int N, int K,
                                   std::vector<uint16_t> &out) {
  const int n_z = N / 32;
  const int iters = K / 8;
  out.assign((size_t)n_z * iters * 256, 0);
  for (int z = 0; z < n_z; ++z) {
    for (int it = 0; it < iters; ++it) {
      uint16_t *blk = out.data() + ((size_t)z * iters + it) * 256;
...
// ============================================================================
// Delegate int4 conv test: compares delegate_conv_int4_wave (int4 weights
// consumed inline via nibble unpack + scale multiply on top of delegate's
// wave-load compute core) against the existing dequant+delegate_conv_wave
// reference pipeline. Reports rel_l2 and TFLOPS across the same 7 Qwen3-4B
// prefill shapes used for fused int4.
// ============================================================================
TEST_F(DelegateConvWaveTest, ModelShapes_DelegateInt4) {
  fprintf(stderr, "\n=== Delegate int4 conv vs dequant+delegate reference ===\n");

  std::string src_int4 = LoadDelegateInt4KernelSource();
  std::string src_dequant = LoadDequantKernelSource();
  std::string src_delegate = LoadKernelSource();
  ...
  cl_kernel k_int4 = build(src_int4,
    "-qcom-accelerate-16-bit=true -cl-std=CL2.0", "delegate_conv_int4_wave");
  cl_kernel k_dequant = build(src_dequant,
    "-cl-std=CL2.0", "dequant_int4_to_delegate_fp16");
  cl_kernel k_delegate = build(src_delegate,
    "-qcom-accelerate-16-bit=true -cl-std=CL2.0", "main_function");
  ...
  double total_ref_us = 0.0;
  double total_int4_us = 0.0;
  ...
  for (const auto &s : kModelShapes) {
    ...
    // Match the RNG scheme used by ModelShapes_Int4Fused so results are
    ...
    // One-off CPU repack into delegate-int4 layout.
    std::vector<uint16_t> repacked_host;
    RepackInt4ForDelegate(packed_host.data(), N, K, repacked_host);
    ...
    // Reference pipeline needs an fp16 weight buffer that dequant writes to.
    cl_mem fp16_weights_dev = cl.clCreateBuffer(ctx, CL_MEM_READ_WRITE,
      (size_t)N * K * 2, nullptr, &e);

    // Candidate pipeline consumes the repacked int4 buffer directly.
    cl_mem repacked_dev = cl.clCreateBuffer(ctx, CL_MEM_READ_ONLY,
      repacked_host.size() * 2, nullptr, &e);
    cl.clEnqueueWriteBuffer(queue, repacked_dev, 1, 0,
      repacked_host.size() * 2, repacked_host.data(), 0, 0, 0);
    ...
    // Shared NDRange used by both delegate-style kernels.
    int4 s0 = {1, dst_slices, M, 32};
    int4 s1 = {src_slices, 0, 0, src_slices};
    int4 s2 = {1, 1, 0, 0};
    size_t gz = (((dst_slices + 7) / 8 + 3) / 4) * 128;
    ...
    // ======= Reference: dequant → fp16 weights → delegate_conv_wave =======
    {
      int sn = N, sk = K;
      cl.clSetKernelArg(k_dequant, 0, sizeof(cl_mem), &packed_dev);
      cl.clSetKernelArg(k_dequant, 1, sizeof(cl_mem), &scales_dev);
    ...
    }
    {
      ...
      total_ref_us += us;
      double tflops = (gflops / (us / 1e6)) / 1000.0;
      fprintf(stderr, "    fp16 ref (dequant once + delegate conv): "
                      "%7.1f us  %.3f TFLOPS\n", us, tflops);
    }
    ...
    cl.clReleaseMemObject(scales_dev);
    cl.clReleaseMemObject(fp16_weights_dev);
    cl.clReleaseMemObject(repacked_dev);
    cl.clReleaseMemObject(xm);
    cl.clReleaseMemObject(si);
    cl.clReleaseMemObject(bias);
    cl.clReleaseMemObject(dst_ref);
  ...
  fprintf(stderr, "    fp16 delegate (ref):       %7.1f us\n", total_ref_us);
  fprintf(stderr, "    int4 delegate (candidate): %7.1f us  (%.2fx vs ref)\n",
          total_int4_us,
          total_int4_us > 0 ? total_ref_us / total_int4_us : 0.0);
