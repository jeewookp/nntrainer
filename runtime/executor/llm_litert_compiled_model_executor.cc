...
// LiteRT per-op profile summary dump at executor destruction. Only runs
// when --enable_op_profiling was set, which caused llm_executor_settings_utils
// to enable runtime_options.SetEnableProfiling(true) at compile time and
// caused the constructor body in the header to retrieve profiler_handle_.
//
// Prints to stderr so it goes straight to temp_run.log alongside the
// existing benchmark_info output from conversation->GetBenchmarkInfo().
// For the benchmark path this fires at engine shutdown, by which point
// prefill has already been dispatched at least once and the internal
// tflite::Profiler buffer has per-op events. The summary format is
// produced by litert/runtime/profiler.cc's LiteRtProfileSummarizer, which
// is the same formatter used by benchmark_litert_model --use_profiler:
// a multi-line table with op type, node index, avg ms, % of total,
// and call count. MatMul / FullyConnected / BatchMatMul ops each get
// their own row, giving us matmul-unit-op granularity.
LlmLiteRtCompiledModelExecutorBase::~LlmLiteRtCompiledModelExecutorBase() {
  if (!op_profiling_enabled_ || profiler_handle_ == nullptr) {
    return;
  }
  const char* summary = nullptr;
  LiteRtStatus status = LiteRtGetProfileSummary(
      profiler_handle_, compiled_model_.Get(), &summary);
  if (status != kLiteRtStatusOk || summary == nullptr) {
    std::fprintf(
        stderr,
        "\n[PROFILE LiteRT per-op] LiteRtGetProfileSummary failed "
        "(status=%d). Did you build with --enable_op_profiling and was "
        "CompiledModel::Create invoked with SetEnableProfiling(true)?\n",
        static_cast<int>(status));
    return;
  }
  std::fprintf(stderr,
               "\n[PROFILE LiteRT CompiledModel per-op summary] "
               "(generated by GetProfileSummary, --enable_op_profiling=true):\n"
               "%s\n",
               summary);
  // API contract: caller frees.
...
absl::Status LlmLiteRtCompiledModelExecutorBase::BindTensorsAndRunPrefill(
  ...
  // LiteRT per-op profiler: arm before dispatch, stop after dispatch. The
  // underlying tflite::Profiler accumulates Begin/End events across every
  // invocation so multiple chunked prefills all contribute to the final
  // summary printed at executor destruction.
  //
  // We use a RAII scope guard so the StopProfiler call always runs even on
  // error return from Run / RunAsync, keeping the profiler state consistent.
  //
  // For the async path StopProfiler is called before the in-flight work
  // finishes; that's fine because the Begin/End events are recorded at op
  // dispatch time on the GPU delegate side via tflite::profiling::Profiler
  // hooks, so StopProfiler only stops ACCEPTING new events. The summary
  // printed from the destructor is fully populated once the engine has
  // joined the inflight work before emitting BenchmarkInfo.
  struct ProfilerScope {
    LiteRtProfiler handle;
    bool enabled;
    ProfilerScope(LiteRtProfiler h, bool en) : handle(h), enabled(en) {
      if (enabled && handle != nullptr) {
        (void)LiteRtStartProfiler(handle);
      }
    }
    ~ProfilerScope() {
      if (enabled && handle != nullptr) {
        (void)LiteRtStopProfiler(handle);
      }
    }
  } profiler_scope(profiler_handle_, op_profiling_enabled_);
