...
Tensor &FloatTensor::dotQInteger(Tensor const &input, Tensor &output,
  ...
  if (input.q_scheme() == QScheme::PER_CHANNEL_AFFINE &&
    ...
    // Input/Output are now FP32 (kernel does FP32<->FP16 conversion internally)
    ...
    // Copy FP32 input data directly to SVM (no conversion, GPU transpose
    // kernel does FP32->FP16 conversion inline)
    ...
    if (!input_cached) {
      ...
      // Copy FP32 data row by row (stride change from K to alignK)
      if (K == alignK) {
        std::memcpy(input_svm, data, M * K * sizeof(float));
      } else {
        for (unsigned int m = 0; m < M; m++) {
    ...
    // Output is now FP32 directly from GEMM kernel - simple memcpy
    ...
    printf("[GPU dotQInt] M=%u K=%u N=%u | alloc=%.2f kai_copy=%.2f repack=%.2f in_copy=%.2f gemm=%.2f out_copy=%.2f | total=%.2f ms\n",
