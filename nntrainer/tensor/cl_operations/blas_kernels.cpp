...
void gemm_int4_cl_adreno(void *input, void *input_transposed, void *weights, void *scales, void *output,
  ...
  if (!result)
    throw std::runtime_error("Failed to set kernel argument 2 for input_transpose");
  int M_4 = ceilDiv(M,4);
  result = kernel_ptr->SetKernelArguments(arg++, &M_4, sizeof(int));
  if (!result)
    throw std::runtime_error("Failed to set kernel argument 3 for input_transpose");
