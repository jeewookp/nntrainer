...
gpu_int4_gemm_adreno(
    ...
    for(int k=0; k<K; k+=4){
        ...
        c0 += convert_float8(input_reg * dq_weights_reg.s0);
        c1 += convert_float8(input_reg * dq_weights_reg.s1);
        c2 += convert_float8(input_reg * dq_weights_reg.s2);
        c3 += convert_float8(input_reg * dq_weights_reg.s3);
    ...
    if (beta == 1) {
        ...
        vstore4(convert_half4(convert_float4(prev) + (float4)(c0.s0, c1.s0, c2.s0, c3.s0)), 0, output + idx); idx += N; }
        if (idx+3<M*N){ prev = vload4(0, output+idx);
        vstore4(convert_half4(convert_float4(prev) + (float4)(c0.s1, c1.s1, c2.s1, c3.s1)), 0, output + idx); idx += N; }
        if (idx+3<M*N){ prev = vload4(0, output+idx);
        vstore4(convert_half4(convert_float4(prev) + (float4)(c0.s2, c1.s2, c2.s2, c3.s2)), 0, output + idx); idx += N; }
        if (idx+3<M*N){ prev = vload4(0, output+idx);
        vstore4(convert_half4(convert_float4(prev) + (float4)(c0.s3, c1.s3, c2.s3, c3.s3)), 0, output + idx); idx += N; }
        if (idx+3<M*N){ prev = vload4(0, output+idx);
        vstore4(convert_half4(convert_float4(prev) + (float4)(c0.s4, c1.s4, c2.s4, c3.s4)), 0, output + idx); idx += N; }
        if (idx+3<M*N){ prev = vload4(0, output+idx);
        vstore4(convert_half4(convert_float4(prev) + (float4)(c0.s5, c1.s5, c2.s5, c3.s5)), 0, output + idx); idx += N; }
        if (idx+3<M*N){ prev = vload4(0, output+idx);
        vstore4(convert_half4(convert_float4(prev) + (float4)(c0.s6, c1.s6, c2.s6, c3.s6)), 0, output + idx); idx += N; }
        if (idx+3<M*N){ prev = vload4(0, output+idx);
        vstore4(convert_half4(convert_float4(prev) + (float4)(c0.s7, c1.s7, c2.s7, c3.s7)), 0, output + idx); }
    } else {
        if (idx+3<M*N){ vstore4(convert_half4((float4)(c0.s0, c1.s0, c2.s0, c3.s0)), 0, output + idx); idx += N; }
        if (idx+3<M*N){ vstore4(convert_half4((float4)(c0.s1, c1.s1, c2.s1, c3.s1)), 0, output + idx); idx += N; }
        if (idx+3<M*N){ vstore4(convert_half4((float4)(c0.s2, c1.s2, c2.s2, c3.s2)), 0, output + idx); idx += N; }
        if (idx+3<M*N){ vstore4(convert_half4((float4)(c0.s3, c1.s3, c2.s3, c3.s3)), 0, output + idx); idx += N; }
        if (idx+3<M*N){ vstore4(convert_half4((float4)(c0.s4, c1.s4, c2.s4, c3.s4)), 0, output + idx); idx += N; }
        if (idx+3<M*N){ vstore4(convert_half4((float4)(c0.s5, c1.s5, c2.s5, c3.s5)), 0, output + idx); idx += N; }
        if (idx+3<M*N){ vstore4(convert_half4((float4)(c0.s6, c1.s6, c2.s6, c3.s6)), 0, output + idx); idx += N; }
        if (idx+3<M*N){ vstore4(convert_half4((float4)(c0.s7, c1.s7, c2.s7, c3.s7)), 0, output + idx); }
    }
}
