// Delegate conv kernel variant that consumes INT4 weights + fp16 scales.
//
// Same high-level architecture as delegate_conv_wave.cl:
//   - 32 output channels per Z-block, 8×half4 accumulators (r0..r7)
//   - 12 concurrent subgroups per workgroup
//   - qcom_sub_group_constant_load8 wave load of 32 half8 (512 B) / iter
//   - 2 src_slices per iter (8 input channels), matches fp16 delegate
// Differences vs fp16 delegate:
//   - weights_buffer holds packed int4 (4 nibbles / ushort) instead of fp16.
//     Only the first 64 ushorts of each 256-ushort load block carry int4
//     data; the remaining 192 are padding to keep the wave-load stride
//     identical to fp16 delegate.
//   - Per-output-channel fp16 scales are passed separately and preloaded
//     into registers (8 × half4) once per Z-block.
//   - Dequant (nibble → (x − 8) × scale) is inlined per FMA, so no fp16
//     staging buffer is needed.
//
// Expected weights_buffer layout:
//   block_idx = Z * iters + it,  iters = K / 8
//   For each block (256 ushorts, i.e. 32 half8):
//     ushorts[ 0..31] = packed_weights_orig[(2*it+0) * N + Z*32 .. Z*32+31]
//     ushorts[32..63] = packed_weights_orig[(2*it+1) * N + Z*32 .. Z*32+31]
//     ushorts[64..255] = padding (don't care)
//   Each ushort packs 4 nibbles for in_ch = (src_slice*4 + c), c=0..3, via
//     nibble(c) = ((u >> (c*4)) & 0xF) - 8
//
// Call site dispatch parameters match delegate_conv_wave.cl exactly:
//   shared_int4_0 = {1, dst_slices, M, 32}
//   shared_int4_1 = {src_slices, 0, 0, src_slices}
//   shared_int4_2 = {1, 1, 0, 0}
// so the host side can swap kernels without changing NDRange shape.

#define MAIN_FUNCTION __kernel void delegate_conv_int4_wave
#define bool2 uchar2
#define bool3 uchar3
#define bool4 uchar4
