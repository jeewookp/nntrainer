// r0..r3 probe of delegate_conv_int4_dense.cl — 1/2 of the compute block.
//
// Sits between probe_delegate_int4_r0.cl (1/8) and delegate_conv_int4_dense.cl
// (8/8). Together the three data points localise the register-pressure cliff
// the full kernel tripped over (694 ms/dispatch vs 554 us for r0 alone).
//
// Identical to delegate_conv_int4_dense.cl except r4..r7 are dropped:
//   - Only r0..r3 accumulated
//   - Only sc0..sc3 preloaded
//   - Only writes coord_s = Z*8 .. Z*8+3 (first 4 out_ch in each Z-block)
//   - Weights ushorts[16..31] and [48..63] are still LOADED by count=8
//     (same f_offset stride) but unused — compare-apples-to-apples on load.
//
// Correctness checked only on the 4-of-8 written output slices.

#define MAIN_FUNCTION __kernel void delegate_conv_int4_dense_r0_r3
#define bool2 uchar2
#define bool3 uchar3
#define bool4 uchar4
...
#define DQ(I,S,SC) (((half)((int)((w_cache[I] >> S) & 0xFu) - 8)) * SC)
  ...
  // Row 0 (src0): r0..r3 using ushorts[0..15].
  ...
  // Row 1 (src1): r0..r3 using ushorts[32..47].
