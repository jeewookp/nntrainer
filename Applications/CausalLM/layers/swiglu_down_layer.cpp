 ...
 * @brief  M=1 decode dispatches swiglu_down_int4_v2 (cooperative
 *         __local-tiled fused kernel).  M>1 prefill computes silu*up
 *         in CPU NEON onto a scratch view of `up_in` and dispatches
 *         the down FC via the standard dot path.
 */
...
void SwigluDownLayer::finalize(nntrainer::InitLayerContext &context) {
  ...
  // Down weight (registered alone -- bundle byte position matches
  // legacy ffn_down).
...
void SwigluDownLayer::incremental_forwarding(
  ...
  nntrainer::TensorDim gate_step_dim = gate_in.getDim();
  ...
  // M=1 fused: kernel does silu(gate)*up cooperatively in __local,
  // then int4 gemv against down weight.
  if (M == 1 &&
    ...
    if (nntrainer::swiglu_down_int4_v2_cl(gate_svm, up_svm, w_svm, s_svm,
                                           out_svm, K, N,
                                           /*sync_output=*/true)) {
      return;
    }
  ...
  // M>1 prefill fallback: CPU NEON silu*up overwrites up_step in
  // place, then standard dot.  up_in's downstream consumers (only
  // this layer in the canonical MLP graph) won't observe the in-place
  // mutation.
  {
