...
private:
  ...
  // weight_idx[0]=gamma (fp32), [1]=up weight (int4), [2]=gate weight (int4).
  // Order preserves the legacy bundle byte layout (ffn_norm.gamma |
  // ffn_up.weight | ffn_gate.weight) so existing pytorch_model.bin
  // loads without repackaging.
