 ...
 * @brief  Fused SwiGLU + down projection for M=1 decode MLP.  Replaces
 *         the swiglu + ffn_down FC pair with a single dispatch:
 *           inputs : (gate_proj_out, up_proj_out) -- both fp16 SVM
 *           weight : down (channel-wise int4) + scale (fp16)
 *           output : decoder hidden (fp16 SVM)
 *
 *         Bundle byte order preserved: this layer registers exactly one
 *         weight (down), in the same byte position the standalone
 *         ffn_down FC used to occupy, so existing pytorch_model.bin
 *         loads unchanged.
 *
 *         M>1 prefill falls back to a sequential swiglu + dot path
 *         (no fused kernel for M>1 yet).
 */
...
private:
  ...
  unsigned int weight_idx; // down weight (int4 channel-wise)
};
