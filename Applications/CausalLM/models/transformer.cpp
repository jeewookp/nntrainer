...
Transformer::createTransformerDecoderBlock(const int layer_id,
  ...
  // ffn_norm rmsnorm absorbed into GateUpLayer (created in createMlp).
  // GateUpLayer registers gamma in the same byte position the
  // standalone rms_norm used to occupy, so the existing
  // pytorch_model.bin loads without repackaging.  Input to GateUpLayer
  // is the residual decoder_add output directly.
