...
# When OP_PROFILING=true, the in-executor shape dumper (called from
# runtime/util/matmul_shape_dump.cc via the executor factory) writes a
# machine-readable CSV of every FC/BatchMatMul/1x1Conv2D op in the
# prefill/decode subgraph to the path in LITERT_LM_MATMUL_ROSTER_CSV.
# This script runs prefill with that env var set, then pulls the CSV
# back to the host as ./matmul_roster.csv so that the standalone
# temp_litert_matmul_micro.sh can later benchmark each shape in
# isolation. The micro benchmark itself lives in its own script now.
...
# to also write a machine-readable CSV next to its stderr dump. We
# pull that CSV back to the host afterwards as ./matmul_roster.csv
# so the standalone temp_litert_matmul_micro.sh can replay it.
...
# Pull the matmul roster CSV back to the host so it survives device
# wipes and is the canonical shape source for ./temp_litert_matmul_micro.sh.
if [ "${OP_PROFILING}" = "true" ]; then
  if adb shell "test -f ${MATMUL_ROSTER_CSV_DEVICE}"; then
    adb pull "${MATMUL_ROSTER_CSV_DEVICE}" "${HOST_MATMUL_ROSTER_CSV}" \
      >/dev/null 2>&1 \
      && echo "[temp_litert.sh] matmul roster CSV pulled to ${HOST_MATMUL_ROSTER_CSV}" \
      || echo "[temp_litert.sh] WARNING: failed to pull matmul roster CSV from device"
  else
    echo "[temp_litert.sh] WARNING: matmul roster CSV not found on device at"
    echo "    ${MATMUL_ROSTER_CSV_DEVICE}"
    echo "    (LITERT_LM_MATMUL_ROSTER_CSV honored? grep '[PROFILE MATMUL SHAPES] CSV roster written to' in ${RUN_LOG})"
  fi
fi
...
# ----------------------------------------------------------------------------
# 6. Done. Matmul micro benchmark is now a separate script:
#    ./temp_litert_matmul_micro.sh
#    It consumes the matmul_roster.csv we just pulled to the host
#    (or the on-device copy as a fallback) and runs matmul_micro_benchmark
#    against each unique (M,N,K) shape from the prefill graph.
# ----------------------------------------------------------------------------
echo ""
echo "Full log: ${RUN_LOG}"
if [ "${OP_PROFILING}" = "true" ] && [ -f "${HOST_MATMUL_ROSTER_CSV}" ]; then
  echo "Matmul roster CSV (host): ${HOST_MATMUL_ROSTER_CSV}"
  echo ""
  echo "Next step (per-shape matmul timing):"
